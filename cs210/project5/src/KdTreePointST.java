...
public class KdTreePointST<Value> implements PointST<Value> {
    ...
    public void put(Point2D p, Value value) {
        ...
        double INF_NEG = Double.NEGATIVE_INFINITY;
        double INF_POS = Double.POSITIVE_INFINITY;
        RectHV rect = new RectHV(INF_NEG, INF_NEG, INF_POS, INF_POS);
    ...
    public Iterable<Point2D> points() {
        // Adapted from pseudocode from April 4th Lecture
        LinkedQueue<Node> traverser = new LinkedQueue<>();
        LinkedQueue<Point2D> collector = new LinkedQueue<>();
        traverser.enqueue(root);

        while (!traverser.isEmpty()) {
            Node x = traverser.dequeue();
            collector.enqueue(x.p);
            if (x.lb != null) {
                traverser.enqueue(x.lb);
            }
            if (x.rt != null) {
                traverser.enqueue(x.rt);
            }
        }
        return collector;
    }
    ...
    private Node put(Node x, Point2D p, Value value, RectHV rect, boolean lr) {
        ...
        if (x.p.equals(p)) {
            ...
            // extrapolate the x or y logic for readability
            boolean compared = lr ? p.x() < x.p.x() : p.y() < x.p.y(); // less or greater?
            if (compared) {
                double newXMax = lr ? x.p.x() : x.rect.xMax();
                double newYMax = lr ? x.rect.yMax() : x.p.y();
                RectHV newRect = new RectHV(x.rect.xMin(), x.rect.yMin(), newXMax, newYMax);
                x.lb = put(x.lb, p, value, newRect, !lr);
            } else {
                double newXMin = lr ? x.p.x() : x.rect.xMin();
                double newYMin = lr ? x.rect.yMin() : x.p.y();
                RectHV newRect = new RectHV(newXMin, newYMin, x.rect.xMax(), x.rect.yMax());
    ...
    private Value get(Node x, Point2D p, boolean lr) {
        ...
        // extrapolate the x or y logic for readability
        boolean compared = lr ? p.x() < x.p.x() : p.y() < x.p.y();
        return get(compared ? x.lb : x.rt, p, !lr);
    }
    ...
    private Point2D nearest(Node x, Point2D p, Point2D nearest, boolean lr) {
        if (x == null || nearest.distanceTo(p) < x.rect.distanceTo(p)) {
            return nearest;
        }
        ...
        Point2D left = nearest(x.lb, p, newNearest, !lr);
        return nearest(x.rt, p, left, !lr);
    }
    ...
    private void nearest(Node x, Point2D p, int k, MaxPQ<Point2D> pq, boolean lr) {
        ...
        if (pq.max().distanceTo(p) >= x.rect.distanceTo(p)) {
